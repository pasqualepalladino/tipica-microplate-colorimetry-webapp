...
def _build_legends_rows(unit_label, mode_flags=None):
    ...
    rows = [
        ...
        {"Field": "SlopeAgreement", "Meaning": "Mean agreement between calibration and standard-addition slopes", "Formula": "mean_k[min(|m_cal|, |m_std,k|) / max(|m_cal|, |m_std,k|)]", "Unit": "dimensionless", "Where used": "METHOD_COMPARISON, Score", "Shown when": stdadd_present, "Notes": "Equals 1 when all available standard-addition slopes match the calibration slope magnitude; lower values indicate poorer agreement."},
        ...
        {"Field": "Score", "Meaning": "Common method-comparison score", "Formula": "For calibration+standard addition rows: slope_agreement^2 x sqrt(R2_cal x R2_std_mean) x (1/LOQ). Calibration-only and standard-addition-only fallback rows are assigned their own ComparableGroup and are not directly ranked against full rows.", "Unit": "dimensionless", "Where used": "OVERVIEW, METHOD_COMPARISON, METHOD_COMPARISON.png", "Shown when": always, "Notes": "Expected/reference values, recovery, SNR and clipping are excluded from the score."},
        ...
        {"Field": "CommonFactorsN", "Meaning": "Number of common factors used in the score", "Formula": "3 for R2_cal, R2_std_mean and SlopeAgreement; LOQ is included in the score formula but not counted in CommonFactorsN; 1 for calibration-only or stdadd-only fallbacks", "Unit": "integer", "Where used": "METHOD_COMPARISON", "Shown when": always, "Notes": "Used to avoid comparing scores obtained from different formulas."},
    ...
    rows.extend([
        ...
        {"Term":"Score", "Meaning":"Common method-ranking score", "Formula":"for calibration plus standard addition: SlopeAgreement^2 x sqrt(R2_cal x R2_std_mean) x (1/LOQ)", "Unit":"1/" + str(unit_label), "Where used":"FIGURE_RGB.png, METHOD_COMPARISON.png, 07_METHOD_COMPARISON", "Shown when":"method comparison present", "Notes":"Expected/reference values, recovery, SNR and clipping are not used in this score."},
...
def _build_method_comparison_rows(fit_rows, selection_info=None, expected_refs=None):
    ...
    # only factors shared by RGB and CIELAB/Delta methods: R2_cal, R2_std_mean and
    ...
    for r in out:
        if np.isfinite(r.get("R2_cal", np.nan)) and np.isfinite(r.get("R2_std_mean", np.nan)) and np.isfinite(r.get("SlopeAgreement", np.nan)):
            ...
            r["CommonFactorsN"] = 3
            r["ScoreFormula"] = "slope_agreement^2 * sqrt(R2_cal * R2_std_mean) * (1/LOQ)"
