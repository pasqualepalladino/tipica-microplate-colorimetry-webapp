 ...
 */
export function rowLabelFromIndex(idx) {
 ...
 */
export function rowIndexFromLabel(label) {
 ...
 */
export function wellId(row, col) {
 ...
 */
export function buildUnitLabel(base, exp) {
    const e = exp.trim();
    if (e === '' || e === '0')
        return base;
    return `${base} 10^${e}`;
 ...
 */
export function parseUnitLabel(label) {
    const sep = ' 10^';
 ...
 */
const NUM_THEN_TYPE_RE = /^([+-]?(?:\d+(?:[.,]\d*)?|[.,]\d+))(UNKNOWN|UNK|U|C|A)(.*)$/i;
/**
 ...
 */
export function normalizeCellTokens(text) {
 ...
 */
function splitOverrideIdDf(tokens) {
    let df = Number.NaN;
    let rest = [...tokens];
 ...
 */
export function parseCellEntry(text, rowIdDefault) {
    ...
    if (!Number.isFinite(concNum))
        return null;
    let type = 'A'; // Python default type
    let overrideTokens = [];
    if (tokens.length >= 2) {
 ...
 */
export function getDefaultIdDf(row, col, defaults, _nrow, _ncol, priority) {
    ...
    if (!id)
        id = rowLabelFromIndex(row) || String(col + 1);
 ...
 *   colId[c]  = str(c+1)
 */
export function buildDefaultPlateDefaults(nrow, ncol) {
    const rowDf = {};
    const rowId = {};
    const colDf = {};
    const colId = {};
...
export function buildEmptyCellGrid() {
    return {};
}
function parseFallbackDf(dfText) {
    const parsed = parseOptionalFloat(dfText);
    return Number.isFinite(parsed) ? parsed : 1;
}
function buildCellText(well, rowDefaultId, rowDefaultDf, includeIdOverride, includeDfOverride) {
    const tokens = [];
 ...
 */
export function wellConfigsToPlateEditorState(wells) {
    const data = wellConfigsToPlateData(wells);
    let nrow = 8;
    let ncol = 12;
    for (const well of wells) {
        if (Number.isFinite(well.row))
            nrow = Math.max(nrow, Math.floor(well.row));
        if (Number.isFinite(well.col))
            ncol = Math.max(ncol, Math.floor(well.col));
    }
    ...
    const uniformRowId = new Map();
    const uniformRowDf = new Map();
    ...
    for (const well of data) {
        ...
        grid[key] = buildCellText(well, rowDefaultId, rowDefaultDf, !hasUniformId, !hasUniformDf);
    }
    return {
        grid,
        defaults,
...
// ---------------------------------------------------------------------------
function plateLabelFromDims(nrow, ncol) {
    for (const label of Object.keys(PLATE_FORMATS)) {
 ...
 */
export function collectPlateState(grid, defaults, nrow, ncol, opts) {
    const data = [];
    for (let r = 0; r < nrow; r++) {
        for (let c = 0; c < ncol; c++) {
            ...
            if (!txt)
                continue;
            const { id: defaultId, df: defaultDf } = getDefaultIdDf(r, c, defaults, nrow, ncol, opts.idDfPriority);
 ...
 */
export function rebuildCellWithTag(text, tag) {
    const tokens = normalizeCellTokens(text);
    if (tokens.length === 0)
        return tag;
 ...
 */
export function applyTagToRow(grid, row, ncol, tag, useStoredCalibration) {
    if (tag === 'C' && useStoredCalibration)
        return grid;
    const next = { ...grid };
    for (let c = 0; c < ncol; c++) {
        const key = `${row}_${c}`;
        ...
        if (useStoredCalibration && cellHasTagC(txt))
            continue;
 ...
 */
export function buildPlateMapTemplateCsv(grid, defaults, nrow, ncol, priority) {
    const lines = ['Well,Conc,Type,ID,DF'];
    for (let r = 0; r < nrow; r++) {
        for (let c = 0; c < ncol; c++) {
            ...
            let df = '';
            if (txt) {
                const { id: defaultId, df: defaultDf } = getDefaultIdDf(r, c, defaults, nrow, ncol, priority);
                const parsed = parseCellEntry(txt, defaultId);
                if (parsed !== null) {
 ...
 */
export function importPlateMapCsv(csvText, nrow, ncol) {
    ...
    const delim = bySemi > byComma && bySemi >= byTab ? ';'
        : byTab > byComma ? '\t'
            : ',';
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (typ === 'UNKNOWN' || typ === 'UNK')
            typ = 'U';
        if (typ !== 'C' && typ !== 'A' && typ !== 'U')
            typ = 'U';
 ...
 */
export function collectExpectedRefs(entries) {
    const result = [];
    const seen = new Set();
 ...
 */
export function plateDataToWellConfigs(data, nrow, ncol) {
 ...
 */
export function wellConfigsToPlateData(wells) {
    const result = [];
